...
export async function sendDeliveryCredentialsEmail(input: CredentialEmailInput) {
  ...
  if (!hasSmtpConfig()) {
    console.warn(
      `[email-fallback] SMTP is not configured. Credentials for ${input.toEmail}: ${input.username} / ${input.password}`,
    );
    return { sent: false };
  }
...
export async function sendDeliveryRegistrationAcknowledgementEmail(
  ...
  if (!hasSmtpConfig()) {
    console.warn(
      `[email-fallback] SMTP is not configured. Registration acknowledgement not sent to ${input.toEmail}.`,
    );
    return { sent: false };
  }
