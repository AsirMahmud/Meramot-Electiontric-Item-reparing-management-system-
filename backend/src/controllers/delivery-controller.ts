...
export async function getDeliveryMe(req: Request, res: Response) {
  try {
    const riderNameId = req.deliveryAuth?.riderNameId;
    if (!riderNameId) {
      return res.status(401).json({ message: "Unauthorized" });
    }

    const rider = await prisma.riderName.findUnique({
      where: { id: riderNameId },
      include: {
        user: {
          select: {
            id: true,
    ...
    return res.json({ riderName: rider });
  } catch (error) {
...
export async function acceptMyDelivery(req: Request, res: Response) {
  try {
    ...
    const existing = await prisma.delivery.findUnique({
      where: { id: deliveryId },
      select: { id: true, riderName: true, status: true },
    });
    ...
    if (existing.riderName && existing.riderName !== riderNameId) {
...
export async function updateLocation(req: Request, res: Response) {
  try {
    ...
    return res.json({ riderName: updated });
  } catch (error) {
...
export async function updateMyDeliveryStatus(req: Request, res: Response) {
  try {
    ...
    const existing = await prisma.delivery.findFirst({
      where: { id: deliveryId, riderName: riderNameId },
      select: { id: true },
    });
...
export async function getDeliveryPayoutSummary(req: Request, res: Response) {
  try {
    ...
    const payouts = await prisma.vendorPayout.findMany({
      where: { riderNameId: summary.riderNameId },
...
export async function requestDeliveryPayout(req: Request, res: Response) {
  try {
    ...
    const created = await prisma.vendorPayout.create({
      data: {
        riderNameId: summary.riderNameId,
