...
function buildCreatedAtFilter(fromValue, toValue) {
...
async function settlePaymentWithCommission(tx, paymentId, settledByAdminId, note) {
...
router.get("/financial-ledger/summary", async (req, res) => {
    try {
        ...
        const [customerPaymentTotal, refundTotal, commissionTotal, vendorPayoutTotal, heldEscrowTotal, pendingSettlementCount,] = await Promise.all([
...
router.get("/financial-ledger/chart-data", async (req, res) => {
    try {
...
router.get("/financial-ledger/entries", async (req, res) => {
    try {
        const action = typeof req.query.action === "string" ? req.query.action.trim() : "";
        const paymentId = typeof req.query.paymentId === "string" ? req.query.paymentId.trim() : "";
        const vendorUserId = typeof req.query.vendorUserId === "string" ? req.query.vendorUserId.trim() : "";
...
router.post("/financial-ledger/settle/:paymentId", async (req, res) => {
    try {
        ...
        const settlement = await prisma.$transaction((tx) => settlePaymentWithCommission(tx, paymentId, req.user.id, req.body?.note));
        return res.json({
            success: true,
...
router.post("/financial-ledger/auto-settle", async (req, res) => {
    try {
        ...
        for (const candidate of candidates) {
            try {
                const result = await prisma.$transaction((tx) => settlePaymentWithCommission(tx, candidate.id, req.user.id, "Auto settlement batch run"));
                settled.push({
...
//# sourceMappingURL=financial-ledger-routes.js.map