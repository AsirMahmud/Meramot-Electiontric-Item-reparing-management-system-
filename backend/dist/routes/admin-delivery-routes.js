...
router.get("/delivery/riders", async (req, res) => {
    try {
...
router.post("/delivery/riders/:userId/approve", async (req, res) => {
    try {
...
router.post("/delivery/riders/:userId/reject", async (req, res) => {
    try {
        const userId = String(req.params.userId);
        const user = await prisma.user.findUnique({
            where: { id: userId, role: "DELIVERY" },
...
//# sourceMappingURL=admin-delivery-routes.js.map