...
export async function getAiChatSessions(req, res) {
    try {
...
export async function createAiChatSession(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
...
export async function saveAiChatMessage(req, res) {
    try {
        const userId = req.user?.id;
...
export async function deleteAiChatSession(req, res) {
    try {
        const userId = req.user?.id;
        const { sessionId } = req.params;
        if (!userId) {
...
//# sourceMappingURL=ai-chat-history-controller.js.map