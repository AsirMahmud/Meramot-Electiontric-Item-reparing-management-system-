// @ts-nocheck
import { BidStatus, DeliveryType, RepairJobStatus, RequestMode, RequestStatus, } from "@prisma/client";
...
function normalizeDeliveryType(input) {
    if (!input)
        return null;
    return Object.values(DeliveryType).includes(input)
        ? input
        : null;
}
function normalizeRequestMode(input) {
    if (input && Object.values(RequestMode).includes(input)) {
        return input;
    }
    return RequestMode.CHECKUP_AND_REPAIR;
}
function normalizeQuoteResponseAction(input) {
...
export async function createRepairRequest(req, res) {
    try {
        ...
        const { title, description, deviceType, brand, model, issueCategory, problem, mode, preferredPickup, deliveryType, shopSlug, imageUrls, } = req.body;
...
export async function acceptRequestBid(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
        ...
        if (acceptedVendorShop?.phone) {
            sendSms(acceptedVendorShop.phone, `Congratulations! Your bid for "${existingRequest.title}" was accepted. Please prepare for the device.`).catch(() => { });
        }
        return res.json({
...
export async function declineRequestBid(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
...
export async function respondToFinalQuote(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const request = await tx.repairRequest.update({
                ...
                data: {
                    ...
                    quotedFinalAmount: existingRequest.repairJob?.finalQuotedAmount ?? existingRequest.quotedFinalAmount,
                },
                select: {
                    id: true,
                    status: true,
        ...
        if (repairJobShop?.phone) {
            ...
            sendSms(repairJobShop.phone, `The customer has ${responseText} your final quote for "${existingRequest.title}".`).catch(() => { });
        }
        return res.json({
            message: approved
