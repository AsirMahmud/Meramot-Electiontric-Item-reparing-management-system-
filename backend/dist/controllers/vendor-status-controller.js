...
async function requireVendorUser(req, res) {
    ...
    try {
        ...
        if (existing.status === "APPROVED") {
            return res.status(400).json({
                message: "Approved vendors must update their public shop details from shop setup or dashboard tools.",
            });
        }
...
export async function completeVendorShopSetup(req, res) {
    try {
        const user = await requireVendorUser(req, res);
        if (!user) {
            return;
        ...
        const inspectionFeeNumber = inspectionFee === "" || inspectionFee === undefined || inspectionFee === null
            ? null
            : Number(inspectionFee);
        const baseLaborFeeNumber = baseLaborFee === "" || baseLaborFee === undefined || baseLaborFee === null
            ? null
            : Number(baseLaborFee);
        const pickupFeeNumber = pickupFee === "" || pickupFee === undefined || pickupFee === null
            ? null
            : Number(pickupFee);
