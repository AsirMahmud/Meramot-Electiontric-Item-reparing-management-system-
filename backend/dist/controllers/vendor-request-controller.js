...
function buildRelevance(request, skillTags) {
    if (!skillTags.length) {
        return {
            isRelevant: true,
            score: 1,
...
async function getVendorContext(userId) {
...
function normalizeVendorJobStatus(input) {
...
function toRequestStatus(jobStatus) {
    switch (jobStatus) {
        case RepairJobStatus.AT_SHOP:
...
function buildAnalyticsBuckets(now, months) {
...
function findBucketIndex(date, buckets) {
    if (!date)
        return -1;
...
export async function getVendorDashboard(req, res) {
    try {
        ...
        const specialtyTokens = shop.specialties
            .flatMap((s) => s.toLowerCase().replace(/[^a-z0-9]+/g, " ").split(" ").filter((t) => t.length > 1));
        ...
        const specialtyFilter = uniqueTokens.length > 0
            ? {
                OR: uniqueTokens.flatMap((token) => [
                    { deviceType: { contains: token, mode: "insensitive" } },
                    { brand: { contains: token, mode: "insensitive" } },
                    { issueCategory: { contains: token, mode: "insensitive" } },
        ...
        return res.json({
            ...
            stats: {
                ...
                waitingApprovalCount: assignedJobs.filter((job) => job.status === RepairJobStatus.WAITING_APPROVAL).length,
                completedJobCount,
            },
            relevantRequests,
            myBids,
    ...
    try {
        ...
        for (const job of jobs) {
            ...
            if (job.status === RepairJobStatus.COMPLETED && job.completedAt) {
                ...
                if (earningsIndex >= 0) {
                    buckets[earningsIndex].earnings = roundMoney(buckets[earningsIndex].earnings + getJobEarnings(job));
        ...
        const bestMonth = buckets.reduce((best, bucket) => {
...
export async function upsertVendorBid(req, res) {
    try {
        const userId = req.user?.id;
        const role = req.user?.role;
        ...
        const { partsCost, laborCost, estimatedDays, notes } = req.body;
        const normalizedPartsCost = normalizeMoney(partsCost, "partsCost");
        const normalizedLaborCost = normalizeMoney(laborCost, "laborCost");
        const normalizedEstimatedDays = normalizeOptionalInteger(estimatedDays, "estimatedDays");
...
export async function updateVendorAssignedJobStatus(req, res) {
    try {
        const userId = req.user?.id;
        const role = req.user?.role;
        ...
        const result = await prisma.$transaction(async (tx) => {
            const repairJob = await tx.repairJob.update({
                ...
                data: {
                    status: nextStatus,
                    startedAt: nextStatus === RepairJobStatus.REPAIRING && !existing.startedAt
                        ? new Date()
                        : existing.startedAt,
                    completedAt: nextStatus === RepairJobStatus.COMPLETED ? new Date() : undefined,
...
export async function submitVendorFinalQuote(req, res) {
    try {
        const userId = req.user?.id;
        const role = req.user?.role;
        const jobId = req.params.jobId;
        ...
        const { shop } = await getVendorContext(userId);
        const { diagnosisNotes, items } = req.body;
        ...
        const finalQuotedAmount = Number(normalizedItems.reduce((sum, item) => sum + item.amount, 0).toFixed(2));
        const existing = await prisma.repairJob.findFirst({
            where: {
                id: jobId,
                shopId: shop.id,
        ...
        if (existing.status === RepairJobStatus.COMPLETED ||
        ...
        const result = await prisma.$transaction(async (tx) => {
            const repairJob = await tx.repairJob.update({
                ...
                data: {
                    ...
                    finalQuotedAmount,
                    finalQuoteItems: normalizedItems,
...
//# sourceMappingURL=vendor-request-controller.js.map