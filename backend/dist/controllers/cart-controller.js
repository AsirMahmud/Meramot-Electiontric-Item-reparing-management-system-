...
export async function getMyActiveCarts(req, res) {
    try {
...
export async function addItemToCart(req, res) {
    try {
        ...
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
        const { shopSlug, serviceName, description, price, quantity, metadata, } = req.body;
        ...
        const cart = await prisma.cart.upsert({
            where: {
                id: (await prisma.cart.findFirst({
                    where: {
                        userId,
                        shopId: shop.id,
                        status: CartStatus.ACTIVE,
        ...
        return res.status(201).json({
            ...
            cart: {
                ...fullCart,
                subtotal: (fullCart?.items || []).reduce((sum, item) => sum + Number(item.price) * item.quantity, 0),
            },
        });
    }
    catch (error) {
...
export async function checkoutCart(req, res) {
    try {
        ...
        const { scheduleType, scheduledAt, paymentMethod, addressMode, address, city, area, lat, lng, deliveryType, problemNote, } = req.body;
        ...
        const chosenAddress = addressMode === "PROFILE"
        ...
        const serviceLines = cart.items
            ...
            .join("\n");
        const title = cart.items.length === 1
