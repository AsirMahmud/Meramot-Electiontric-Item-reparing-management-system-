...
function subjectForStatus(status, orderRef) {
    const normalized = status.toUpperCase();
    if (normalized === "PROCESSING")
        return `Your Meramot order ${orderRef} is being processed`;
    if (normalized === "ON_THE_WAY" || normalized === "RETURNING")
        return `Your Meramot order ${orderRef} is on the way`;
    if (normalized === "COMPLETED")
        return `Your Meramot order ${orderRef} is complete`;
    return `Update for your Meramot order ${orderRef}`;
}
function htmlForStatus({ customerName, orderRef, status, shopName }) {
...
function htmlPaymentReceipt({ customerName, transactionRef, amount, currency, invoiceUrl }) {
    const greeting = customerName ? `Hi ${customerName},` : "Hello,";
    return `
...
export async function sendInvoiceLinkEmail(input) {
    if (!env.enableEmailNotifications) {
        return { ok: false, skipped: true, reason: "email notifications disabled" };
    }
    if (!env.resendApiKey) {
