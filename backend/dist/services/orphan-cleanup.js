...
async function cleanupOrphanedAccounts() {
    try {
        ...
        console.log(`[cleanup] Removed ${result} orphaned account(s) from rejected vendor applications older than ${ORPHAN_AGE_DAYS} days.`);
...
export function startOrphanCleanupScheduler() {
    ...
    console.log(`[cleanup] Orphaned account cleanup scheduled (every ${CLEANUP_INTERVAL_MS / 3600000}h, accounts older than ${ORPHAN_AGE_DAYS} days)`);
}
/**
