...
export default function AuthCard({ mode }: { mode: Mode }) {
  ...
  const passwordBar = useMemo(() => getPasswordBar(form.password), [form.password]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isSignup) {
        ...
        if (usernameStatus === "taken") throw new Error("Please choose a different username.");
        if (!passwordBar.acceptable) throw new Error("Please choose a stronger password.");
          ...
          <>
            ...
            <div>
              ...
              {usernameStatus === "checking" && (
                <p className="mt-1 text-xs text-muted-foreground">Checking username...</p>
              )}
              {usernameStatus === "available" && (
                <p className="mt-1 text-xs text-green-600">Username is available.</p>
              )}
              {usernameStatus === "taken" && (
                <p className="mt-1 text-xs text-red-600">Username already in use.</p>
              )}
            </div>
          </>
        )}
