          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[var(--muted-foreground)]">
                <span className="whitespace-nowrap">⭐ {shop.ratingAvg.toFixed(1)}</span>
                <span className="whitespace-nowrap">({shop.reviewCount})</span>
                {typeof shop.distanceKm === "number" ? (
                  <span className="whitespace-nowrap">{shop.distanceKm.toFixed(1)} km away</span>
                ) : null}
                <span className="whitespace-nowrap">{etaLabel(shop.etaMinutes)}</span>
              </div>
            </div>

            <div className="ml-2 text-right">
              <div className="text-xl sm:text-2xl font-extrabold leading-tight text-[var(--accent-dark)] break-words">
                {shop.offerSummary ?? "৳--"}
              </div>
