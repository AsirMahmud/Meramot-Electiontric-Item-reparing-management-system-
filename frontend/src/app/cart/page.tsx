...
export default function CartPage() {
  ...
  const [apiCarts, setApiCarts] = useState<Cart[]>([]);
  const guestCart = useGuestCart();
  const carts = isLoggedIn ? apiCarts : guestCart.carts;
  ...
  async function loadCart() {
    if (!isLoggedIn || !token) {
      setMessage(guestCart.carts.length ? "" : "Your cart is empty.");
      return;
    }
  ...
  function removeGuestItem(itemId: string) {
    guestCart.removeCartItem(itemId);
  }
  ...
  async function handleConfirmOrder() {
    ...
    if (addressMode === "MANUAL" && !address.trim()) {
      setMessage("Please add a pickup address before confirming. Use the manual address below.");
      return;
    }

    if (addressMode === "MAP" && !address.trim() && lat === undefined && lng === undefined) {
      setMessage("Please select a valid location from the map or use manual address.");
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 pb-32 md:px-6 xl:pb-8">
        ...
        {message ? (
          <div
            className={`mt-6 rounded-[1.6rem] border px-5 py-4 text-sm font-semibold shadow-sm ${message.toLowerCase().includes("failed") ||
    ...
    `}
                >
                  ...
                  {/* Content */}
                  <div
                    className={`flex-1 overflow-y-auto px-4 pb-6 transition ${summaryOpen ? "opacity-100" : "opacity-0 pointer-events-none"
                      }`}
                  >
