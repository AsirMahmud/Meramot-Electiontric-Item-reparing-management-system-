...
export default function ProfilePage() {
  ...
  const { startUpload } = useUploadThing("userAvatarUploader");
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col items-center gap-3">
              {profile?.avatarUrl || draft?.avatarUrl ? (
                <div className="relative h-20 w-20 overflow-hidden rounded-3xl border border-[var(--border)] shadow-sm">
                  <Image
              ...
              {editing && (
                <div className="w-full flex justify-center mt-2">
                  <label className="cursor-pointer rounded-full bg-[var(--mint-100)] px-4 py-2 text-xs font-semibold text-[var(--accent-dark)] transition hover:bg-[var(--mint-200)]">
                    {draft?.avatarUrl || profile?.avatarUrl ? "Change picture" : "Upload picture"}
                    <input
                      type="file"
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          setLocalFile(file);
                          setDraft((prev) => ({ ...prev, avatarUrl: URL.createObjectURL(file) }));
                        }
                      }}
                    />
                  </label>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      // If there is a new local file, upload it first
                      if (localFile) {
                        const res = await startUpload([localFile]);
                        if (res && res[0]) {
                      ...
                      const payloadToSave = { ...draft, avatarUrl: finalAvatarUrl };

                      const updated: any = await updateProfile(token, payloadToSave);
                  ...
                  className="rounded-full bg-[var(--accent-dark)] px-6 py-3 text-sm font-semibold text-white disabled:opacity-50 transition"
                >
